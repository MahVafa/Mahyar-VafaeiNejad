...
Ser = 20
np.random.seed(seed = 1)
C = np.random.randint(1000,2000,(Ser))
R = np.random.randint(400,700,(Ser))  # Service Radius
np.random.seed(seed = 1)
...
F = np.random.randint(150,350,Ser)
...
def Hub_Cost(S):
    ...
    for i in range(Cities):
       for j in range(np.size(S)):
           if S[j] == 1 and math.sqrt((xs[i]-xh[j])**2+(ys[i]-yh[j])**2) <= R[j]:
              distance[i,j] = math.sqrt((xs[i]-xh[j])**2 + (ys[i]-yh[j])**2)
           else:
              distance[i,j] = np.inf
    ...
    for i in range(Cities):
       for j in range(np.size(S)):
           if d[i,1] == j:
             C_R[j] -= D[i]    
    for j in range(np.size(C_R)):
        if C_R[j] < 0:
            c += 1
    if (np.sum(S) > 10) or (np.sum(S) == 0) or (c > 0):
      Z1 = np.inf
      Z2 = np.inf
      Z =  np.inf
    else:
      Z1 = np.round(np.sum(F * S),0)
      Z2 = np.round(np.max(d[:,0]),0)
      Z = np.round(4 * Z1 + 0.5 * Z2)  
    return Z, Z1, Z2, C_R, c, S
# In[] Crossovers
def SPCO(P1,P2):
    n = np.size(P1)
...
for j in range(np.size(Bestsol)):
    if 1 in Bestsol and Actives[j] == 1:
        cir = Circle((xh[j], yh[j]), radius=R[j],facecolor='none',edgecolor=(0,0.3,0.8),alpha=0.6,linestyle=':')
        ax.add_patch(cir)
        ax.autoscale()
        for i in range(Cities):
            plt.plot([xs[i],xh[int(d[i,1])]],[ys[i],yh[int(d[i,1])]],'black',linestyle='--',markersize=9)
